...
def assegna_priorita(titolo, descrizione, priorita_originale):
    ...
    parole_alta = ["urgente","bloccato","blocco","scadenza","reclamo",
                   "anomalia","malfunzionamento","errore","rottura",
                   "sollecito"
    ]

    parole_media = [ "verifica","revisione","aggiornamento","ristampa","spedizione",
                    "campionatura","variazione","chiarimento"
    ]

    parole_bassa = ["riepilogo","copia","consulenza",
                    "informazioni","documentazione"
    ]

    if any(parola in testo for parola in parole_alta):
        punteggio += 2
    if any(parola in testo for parola in parole_media):
        punteggio += 1
    if any(parola in testo for parola in parole_bassa):
        punteggio -= 1
    if punteggio >= 2:
        return "alta"
    elif punteggio == 1:
