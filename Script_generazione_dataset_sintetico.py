...
def carica_categorie():
    try: 
        with open('tickets.json', 'r', encoding='utf-8') as documento:
...
def genera_dataset_sintetico(categorie, lenght =TICKET_PER_CATEGORIA):
    nomi_categorie = list(categorie.keys())
    totale_ticket = lenght * len(nomi_categorie)
    ...
    for nome_categoria in nomi_categorie:
        ...
        while ticket_generati < lenght: 
            info_templates = random.choice(informazioni["ticket_templates"]) #Sceglie casualmente un template di informazioni per la categoria
...
def esporta_dataset(ticket,nome_iniziale = 'dataset_ticket', stratify_col = 'category', seed = 1):
    ...
    try:
        ...
        print("Errore durante il salvataggio dei file CSV: {e}")
        return None
...
def valida_coerenza_ticket(titolo, descrizione, categoria, categorie):
    ...
    for parola_chiave in parole_chiave:
        if parola_chiave.lower() in combina_testo:
            parole_count += 1 #Incrementa il contatore

    for parola_correlata in parole_correlate:
        if parola_correlata.lower() in combina_testo:
            parole_count += 1
...
def assegna_priorita(titolo, descrizione, priorita_originale):
    combinato_testo = (titolo + " " + descrizione).lower()
    ...
    #Segnali alta priorità = +2 punti
    alta_parole = ["annullamento", "reclamo", "richiesta", "bloccato", "proposta", "trattativa", 
                   "sblocco", "riattivazione", "accredito", "sollecito", "urgente", "storno",
                   "anomalia", "rottura", "colori", "layout", "vettoriale", "anteprima", "bozzetto"
    ]
    if any(w in combinato_testo for w in alta_parole):
        punteggio += 2
   
    # Segnali Alta priorità = +1 punto
    alta_moderate = ["pagamento", "bonifico", "scadenza", "anomalia", "malfunzionamento",
                     "errore", "lento", "rallentamento", "performance", "timeout",
                     "sincronizzazione"]
    if any(w in combinato_testo for w in alta_moderate):
        punteggio += 1
   
    # Segnali Media Priorità = -1 punti
    media_parole = [
        "revisione", "chiarimento", "ristampa", "offerta", "campionatura", "ripristino",
        "spedizione", "prenotazione", "scadenze", "contabile", "listino", 
        "scontistica", "variazione"
    ]
    if any(w in combinato_testo for w in media_parole):
        punteggio -= 1
   
    # Segnali Bassa Priorità = -2 punti
    bassa_parole = ["proposta", "commerciale", "riepilogo", "copia", "nota di credito",
                    "scheda", "asciugatura", "lavorazione", "aggiornamento",
                    "consulenza", "arrivo"
    ]
    if any(w in combinato_testo for w in bassa_parole):
        punteggio -= 2

    #Variazione casuale (+1, 0, o -1)
    rumore =random.choice([-1,0,1])
    punteggio += rumore
       
    # Decisione
    if punteggio >= 3:
        return "alta" if random.random() < 0.80 else "media"
   
    elif punteggio <= -3:
        return "bassa" if random.random() < 0.80 else "media"
   
    elif punteggio >= 1:
        return "alta" if random.random() < 0.60 else "media"
   
    elif punteggio <= -1:
        return "bassa" if random.random() < 0.60 else "media"
...
def genera_varianti_ticket(titolo, descrizione, categoria, categorie):
    # Creazione della prima variante di ticket (originale)
    variante1 = {
        'titolo': titolo,
    ...
    # Creazione della seconda variante di ticket (sostituzione con parole correlate)
    variante2_titolo = titolo
    variante2_descrizione = descrizione

    # Verifica la presenza delle categorie e parole correlate nel Json, se ok estrazione parole correlate per la categoria
    if categoria in categorie and 'parole_correlate' in categorie[categoria]:
        ...
        # Itera sulle coppie del dizionario e sceglie casualmente una parola correlata
        for parola_originale, lista_parole_correlate in parole_correlate_categoria.items():
            sostituzione = random.choice(lista_parole_correlate)

            # Sostituzione nel titolo
            if parola_originale.lower() in variante2_titolo.lower():
                originale = parola_originale
                for verifica_parola in variante2_titolo.split():
                    if verifica_parola.lower().startswith(originale.lower()):
                        originale = verifica_parola
                        break # Ferma dopo la prima sostituzione trovata
                variante2_titolo = variante2_titolo.replace(originale, sostituzione, 1)

            # Sostituzione descrizione
            elif parola_originale.lower() in variante2_descrizione.lower():
                originale = parola_originale
                for verifica_parola in variante2_descrizione.split():
                    if verifica_parola.lower().startswith(originale.lower()):
                        originale = verifica_parola
                        break # Ferma dopo la prima sostituzione trovata
                variante2_descrizione = variante2_descrizione.replace(originale, sostituzione, 1)
            break
    # Inseriamo nel dizionario
    variante2 = {
...
if __name__ == '__main__':
    try:
        ...
        ticket_generati = genera_dataset_sintetico(categorie, TICKET_PER_CATEGORIA)
